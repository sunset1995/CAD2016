...
Circuit ISC_parser::parse_isc_file(const char *filename) {
	...
	while( fgets(str, 256, in)!=nullptr ) {
		...
		if     ( strstr(str, "INPUT")!=nullptr )
			ret.insert_gate(0, -1, -1, -1, signal_id[0]);
		else if( strstr(str, "OUTPUT")!=nullptr )
			ret.insert_gate(1, -1, -1, -1, signal_id[0]);
		else if( strstr(str, "NAND")!=nullptr )
			ret.insert_gate(3, signal_id[1], signal_id[2], -1, signal_id[0]);
		else if( strstr(str, "NOR")!=nullptr )
			ret.insert_gate(5, signal_id[1], signal_id[2], -1, signal_id[0]);
		else if( strstr(str, "NXOR")!=nullptr )
			ret.insert_gate(7, signal_id[1], signal_id[2], -1, signal_id[0]);
		else if( strstr(str, "XOR")!=nullptr )
			ret.insert_gate(6, signal_id[1], signal_id[2], -1, signal_id[0]);
		else if( strstr(str, "AND")!=nullptr )
			ret.insert_gate(2, signal_id[1], signal_id[2], -1, signal_id[0]);
		else if( strstr(str, "OR")!=nullptr )
			ret.insert_gate(4, signal_id[1], signal_id[2], -1, signal_id[0]);
		else if( strstr(str, "BUFF")!=nullptr )
			ret.insert_gate(8, signal_id[1], -1, -1, signal_id[0]);
		else if( strstr(str, "NOT")!=nullptr )
			ret.insert_gate(9, signal_id[1], -1, -1, signal_id[0]);
		else if( strstr(str, "DFF")!=nullptr )
			ret.insert_gate(10, signal_id[2], signal_id[3], signal_id[4], signal_id[0]);
	}
	fclose(in);
